...
def dropsand(grid,sandpoint):
    ...
    while 1:
        ...
        if grid[sy+1][sx+1]=='.': sx+=1; sy+=1; continue
        if sy==sandpoint[1]:blocked=True;break
        else: break
    grid[sy][sx]='o'
...
minx,miny,maxx,maxy=minmax(scanline)
wallline=[[minx-(maxy+10),maxy+2],[maxx+(maxy+10),maxy+2]]
scanline.append(wallline)
#print(scanline)
...
while 1:#contained==True:
    ...
    grid,blocked=dropsand(grid,sandpoint)
    if blocked==True: total=counter;break
